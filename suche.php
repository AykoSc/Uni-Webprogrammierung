    ...
    if ($_GET["herkunft"] === "ausstellung") {
        ...
        foreach ($ausstellung as $reihe) {
            foreach ($reihe as $gemaelde) {
                ...
                $ergebnis .= "<a href='gemaelde.php?id=" . urlencode($gemaelde[0]) . "'>" . "<img id='suchlupe' src='images/suche_schwarz.svg' alt='suche' width='13' height='13'>" . urlencode($gemaelde[2]) . "</a>";
                $ergebniszaehler++;
        ...
        foreach ($sammlungen as $reihe) {
            foreach ($reihe as $sammlung) {
                if ($ergebnis !== "") $ergebnis .= "<br/>";
                $ergebnis .= "<a href='sammlung.php?id=" . urlencode($sammlung[0]) . "'>" . "<img id='suchlupe' src='images/suche_schwarz.svg' alt='suche' width='13' height='13'>" . urlencode($sammlung[3]) . "</a>";
                $ergebniszaehler++;
                if ($ergebniszaehler == 10) break 2;
            }
        }
