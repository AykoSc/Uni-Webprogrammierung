    ...
    if ($_GET["herkunft"] === "ausstellung") {
        ...
        foreach ($sammlungen as $reihe) {
            foreach ($reihe as $sammlung) {
                if ($ergebnis !== "") $ergebnis .= "<br/>";
                $ergebnis .= "<a href='sammlung.php?id=" . htmlspecialchars($sammlung[0]) . "'>" . "<img style='margin-right: 15px' src='images/sucheschwarz.svg' alt='suche' width='13' height='13'>" . htmlspecialchars($sammlung[3]) . "</a>";
                $ergebniszaehler++;
                if ($ergebniszaehler == 10) break 2;
            }
        }
